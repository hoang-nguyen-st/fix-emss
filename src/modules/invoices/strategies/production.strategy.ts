...
@Injectable()
export class ProductionStrategy extends InvoiceCalculationBaseStrategy {
  async calculate(
    ...
    for (let i = 0; i < devices.length; i++) {
      ...
      if (!ltvl) {
        throw new NotFoundException(
          `Không tìm thấy quy tắc điện áp cho loại hình thức SẢN XUẤT và mức điện áp ${matchingVoltageLevel.name}. Vui lòng kiểm tra bảng location_type_voltage_levels.`
        );
      }
      ...
      if (!pricingRule) {
        throw new NotFoundException(
          `Không tìm thấy đơn giá cho workspace và loại hình thức SẢN XUẤT với mức điện áp ${matchingVoltageLevel.name}. Vui lòng kiểm tra bảng pricing_electric_rules.`
        );
      }
